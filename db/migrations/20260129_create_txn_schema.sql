...
-- Down: Drop triggers, functions, tables, enums
-- Note: Reversing should be done carefully in production.

-- To rollback run the following (order matters):
-- DROP TRIGGERS
DROP TRIGGER IF EXISTS trg_recalc_total ON transaction_lines;
DROP TRIGGER IF EXISTS trg_enforce_line_amount ON transaction_lines;
DROP TRIGGER IF EXISTS trg_txn_immutable ON transactions;
DROP TRIGGER IF EXISTS trg_metadata_transactions ON transactions;
DROP TRIGGER IF EXISTS trg_metadata_lines ON transaction_lines;
DROP TRIGGER IF EXISTS trg_metadata_entities ON entities;
DROP TRIGGER IF EXISTS trg_phone_entities ON entities;
DROP TRIGGER IF EXISTS trg_phone_users ON users;

-- DROP FUNCTIONS
DROP FUNCTION IF EXISTS recalc_txn_total();
DROP FUNCTION IF EXISTS enforce_line_amount();
DROP FUNCTION IF EXISTS prevent_immutable_changes();
DROP FUNCTION IF EXISTS check_metadata_snakecase();
DROP FUNCTION IF EXISTS phone_number_format_check();
...
-- Drop extension pgcrypto if you want (left intentionally)
