...
export class InventoryIntelligence {
     ...
     */
    static predictDaysOfStockLeft(item: { currentStock: number; unit: string }, salesHistory: ItemSalesData[]): number {
        if (salesHistory.length === 0) return 999; // Assume plenty if no sales history
     ...
     */
    static getStockAction(item: { name: string; currentStock: number; minStock?: number; unit: string }, daysLeft: number): string {
     ...
     */
    static detectWastageAlert(item: { name: string; isPerishable: boolean; shelfLifeDays?: number; lastStockUpdate?: Date }, daysLeft: number): string | null {
        if (!item.isPerishable || !item.shelfLifeDays || !item.lastStockUpdate) return null;
