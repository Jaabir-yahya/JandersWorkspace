 ...
 */
const FIXED_HOLIDAYS: Array<{ name: string; swahiliName: string; month: number; day: number; description?: string }> = [
 ...
 */
function calculateEasterMonday(year: number): Date {
  const easter = calculateEaster(year);
 ...
 */
function calculateIddUlFitr(year: number): Date {
  // Approximate dates for Idd-ul-Fitr (Islamic calendar shifts ~11 days earlier each Gregorian year)
  const approximateDates: Record<number, { month: number; day: number }> = {
 ...
 */
function calculateIddUlAdha(year: number): Date {
 ...
 */
export function getKenyanHolidays(year: number): KenyanHoliday[] {
 ...
 */
export function getNextBusinessDay(date: Date, daysToAdd: number = 1): Date {
 ...
 */
export function getPreviousBusinessDay(date: Date, daysToSubtract: number = 1): Date {
  const result = new Date(date);
 ...
 */
export function getHolidaysInRange(startDate: Date, endDate: Date): KenyanHoliday[] {
  const startYear = startDate.getFullYear();
  const endYear = endDate.getFullYear();
  const allHolidays: KenyanHoliday[] = [];
 ...
 */
export function getHolidayName(date: Date): { name: string; swahiliName: string } | null {
  const year = date.getFullYear();
  const holidays = getKenyanHolidays(year);
 ...
 */
export function getUpcomingHolidays(fromDate: Date = new Date(), count: number = 5): KenyanHoliday[] {
