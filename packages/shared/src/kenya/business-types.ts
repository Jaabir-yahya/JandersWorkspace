 ...
 */
export type KenyanBusinessType =
  | 'sole_proprietorship'
  | 'partnership'
  | 'limited_company'
 ...
 */
export type KenyanBusinessSector =
  | 'agriculture'
  | 'manufacturing'
  | 'retail'
  | 'wholesale'
 ...
 */
export const KENYA_MSME_CLASSIFICATIONS: Record<string, MSMEClassification> = {
  micro: {
    category: 'micro',
    maxEmployees: 10,
 ...
 */
export const KENYAN_BUSINESS_TYPES: Record<KenyanBusinessType, BusinessTypeInfo> = {
  sole_proprietorship: {
 ...
 */
export const KENYAN_BUSINESS_SECTORS: Record<KenyanBusinessSector, { name: string; swahiliName: string; kraprefix: string }> = {
...
export function classifyMSME(
  ...
  if (
    annualTurnover <= KENYA_MSME_CLASSIFICATIONS.micro.maxAnnualTurnover ||
    employeeCount <= KENYA_MSME_CLASSIFICATIONS.micro.maxEmployees
  ) {
    return KENYA_MSME_CLASSIFICATIONS.micro;
  }

  if (
    annualTurnover <= KENYA_MSME_CLASSIFICATIONS.small.maxAnnualTurnover ||
    employeeCount <= KENYA_MSME_CLASSIFICATIONS.small.maxEmployees
  ) {
    return KENYA_MSME_CLASSIFICATIONS.small;
  }

  if (
    annualTurnover <= KENYA_MSME_CLASSIFICATIONS.medium.maxAnnualTurnover ||
    employeeCount <= KENYA_MSME_CLASSIFICATIONS.medium.maxEmployees
  ) {
    return KENYA_MSME_CLASSIFICATIONS.medium;
  }
 ...
 */
export function getBusinessTypeInfo(type: KenyanBusinessType): BusinessTypeInfo {
  return KENYAN_BUSINESS_TYPES[type];
}
 ...
 */
export function getSectorInfo(sector: KenyanBusinessSector): { name: string; swahiliName: string; kraprefix: string } {
  return KENYAN_BUSINESS_SECTORS[sector];
}
 ...
 */
export function requiresBusinessVatRegistration(annualTurnover: number): boolean {
