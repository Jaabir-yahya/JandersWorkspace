...
# Complete Test Data Population for Phase 3
# This script creates test data with proper user creation
...
echo "=========================================="
echo "Phase 3 Complete Test Data Population"
echo "=========================================="
echo ""
...
echo -e "${BLUE}Step 0: Creating User${NC}"
echo "----------------------------------------"
...
echo -e "${GREEN}✓${NC} Created user: System Admin ($USER_ID)"
echo ""

if [ -z "$USER_ID" ]; then
    echo "ERROR: Failed to create user. Cannot proceed."
    exit 1
fi
...
RESPONSE=$(curl -s -X POST "$API_URL/entities" \
    ...
    -d "{
        ...
        \"metadata\": {\"email\": \"john@example.com\", \"address\": \"Nairobi, Kenya\", \"tax_id\": \"TAX-001\"},
...
RESPONSE=$(curl -s -X POST "$API_URL/entities" \
    ...
    -d "{
        ...
        \"metadata\": {\"email\": \"mary@example.com\", \"address\": \"Mombasa, Kenya\", \"tax_id\": \"TAX-002\"},
        \"created_by_user_id\": \"$USER_ID\"
    }")
echo "Response: $RESPONSE"
...
RESPONSE=$(curl -s -X POST "$API_URL/entities" \
    ...
    -d "{
        \"tenant_id\": \"$TENANT_ID\",
        \"display_name\": \"ABC Electronics Ltd\",
        \"type\": \"SUPPLIER\",
        \"phone_number\": \"+254745678901\",
        \"metadata\": {\"email\": \"sales@abcelectronics.com\", \"address\": \"Industrial Area, Nairobi\", \"tax_id\": \"TAX-003\"},
        \"created_by_user_id\": \"$USER_ID\"
    }")
echo "Response: $RESPONSE"
        ...
        \"lines\": [
            {
                ...
                \"metadata\": {\"product_name\": \"Laptop Stand\"}
            }
        ]
    }")
echo "Response: $RESPONSE"
        ...
        \"lines\": [
            {
                ...
                \"unit_price\": 5000,
                \"account_code\": \"200-SALES\",
                \"metadata\": {\"product_name\": \"Wireless Mouse\"}
            }
        ]
    }")
echo "Response: $RESPONSE"
        ...
        \"lines\": [
            {
                ...
                \"unit_price\": 2500,
                \"account_code\": \"200-SALES\",
                \"metadata\": {\"service_type\": \"CONSULTING\", \"hours\": 10, \"hourly_rate\": 2500}
            }
        ]
    }")
echo "Response: $RESPONSE"
...
RESPONSE=$(curl -s -X POST "$API_URL/transactions" \
    ...
    -d "{
        ...
        \"lines\": [
            {
                \"description\": \"Projector rental (7 days)\",
                \"quantity\": 1,
                \"unit_price\": 12000,
                \"account_code\": \"200-SALES\",
                \"metadata\": {\"item_name\": \"Projector\", \"rental_period_days\": 7, \"daily_rate\": 1500, \"deposit\": 5000, \"return_date\": \"2026-02-05\"}
            }
        ]
    }")
echo "Response: $RESPONSE"
...
RESPONSE=$(curl -s "$API_URL/transactions?tenant_id=$TENANT_ID&type=RETAIL")
echo "Response: $RESPONSE"
...
RESPONSE=$(curl -s "$API_URL/transactions?tenant_id=$TENANT_ID&type=SERVICE")
echo "Response: $RESPONSE"
...
RESPONSE=$(curl -s "$API_URL/transactions?tenant_id=$TENANT_ID&type=RENTAL")
echo "Response: $RESPONSE"
