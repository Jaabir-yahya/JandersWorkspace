...
export default function SuppliesPage() {
  ...
  useEffect(() => {
    containersApi
      .list()
      .then((list) =>
        setContainers(list.map((c) => ({ id: c.id, name: c.name }))),
      )
      .catch(() => setContainers([]));
  }, []);
  ...
  const updateItem = (index: number, field: keyof PurchaseItem, value: any) => {
    ...
    newItems[index].total =
      newItems[index].quantity * (newItems[index].unitCost ?? 0);
  ...
  const handleSubmit = async () => {
    ...
    if (
      formData.items.some((item) => !item.supplyItemId || item.quantity <= 0)
    ) {
            ...
            <CardBody className="space-y-4">
              {formData.items.length === 0 ? (
                ...
                formData.items.map((item, index) => (
                  <div
                    key={index}
                    className="p-4 bg-savanna-50 rounded-lg space-y-3"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-baobab-700">
                        Item #{index + 1}
                      </span>
                      <button
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        updateItem(index, "supplyItemId", e.target.value)
                      }
                      options={[
                        { value: "", label: "Select item..." },
                        ...supplyItems.map((si) => ({
                          value: si.id,
                          label: `${si.name} (${si.code}) - ${formatCurrency(si.unitCost ?? 0, (si.currency ?? "KES") as Currency)}/${si.unit ?? ""}`,
                        })),
                      ]}
                    />
              ...
              {formData.paymentMethod === "MPESA" && (
                <Input
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      paymentReference: e.target.value.toUpperCase(),
                    })
                  }
                  required
                />
          ...
          <div className="sticky top-24">
            ...
            {ledgerEntries.length > 0 && (
              <div className="mt-4 p-4 bg-acacia-50 border border-acacia-200 rounded-lg">
                <h3 className="font-medium text-acacia-800 mb-2">
                  What happens:
                </h3>
